...
const detailform = async (req, res) => {
  ...
  var { php, tphp, mysql, tmysql, oracle, toracle, laravel, tlaravel } =
    req.body;
  ...
  result[0].forEach((data) => {
    ...
    if (data.optionname == "gujarati" && gujarati == "gujarati")
      gujarati = data.id;
    ...
    if (data.optionname == "begineer " && toracle == "begineer")
      toracle = data.id;
    if (data.optionname == "expert " && toracle == "expert") toracle = data.id;
    if (data.optionname == "mediator " && toracle == "mediator")
      toracle = data.id;

    if (data.optionname == "begineer " && tmysql == "begineer")
      tmysql = data.id;
    if (data.optionname == "expert " && tmysql == "expert") tmysql = data.id;
    if (data.optionname == "mediator " && tmysql == "mediator")
      tmysql = data.id;

    if (data.optionname == "begineer " && tlaravel == "begineer")
      tlaravel = data.id;
    if (data.optionname == "expert " && tlaravel == "expert")
      tlaravel = data.id;
    if (data.optionname == "mediator " && tlaravel == "mediator")
      tlaravel = data.id;
  });

  var e_id = 0;
  var sql1 =
    "INSERT INTO basic_details (`firstname`, `lastname`, `designation`, `address_1`, `address2`, `email`, `phone_number`, `city`, `state`, `gender`, `zipcode`, `relationship`, `dateofbirth`) VALUES (?,?, ?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
  var basic = await con
    .promise()
    .query(sql1, [
      fname,
  ...
  var sql3 =
    "INSERT INTO `educationdetails` (`employeeid`, `typeofed`,`coursename`, `passingyear`,`percentage`) VALUES (?, ?,?,?,?);";
  ...
  var sql3 =
    "INSERT INTO `workexp` (`employeeid`, `Company_name`, `Designation`, `from_d`, `to_d`) VALUES (?, ?, ?, ?, ?);";
  for (let i = 0; i < CompanyName.length; i++) {
    await con
      .promise()
      .query(sql3, [e_id, CompanyName[i], designation1[i], From1[i], To1[i]]);
  }

  var sql3 =
    "INSERT INTO `referance` (`employeeid`, `refrencename`, `contact`, `relation`) VALUES ( ?, ?, ?, ?);";
  for (let i = 0; i < nameref.length; i++) {
    await con
      .promise()
      .query(sql3, [e_id, nameref[i], Contactnumber[i], Relation[i]]);
  }

  var sql3 =
    "INSERT INTO `language` (`optionid`, `employeeid`, `reade`, `writee`, `speak`) VALUES (?, ?, ?,?,?)";
  if (hindi) {
    if (hindi_r == "on") {
      hindi_r = 1;
    } else {
  ...
  if (gujarati) {
    ...
    await con
      .promise()
      .query(sql3, [7, e_id, gujarati_r, gujarati_w, gujarati_s]);
  }

  var sql4 =
    "INSERT INTO `technology` (`employeeid`, `technology`, `tech_level`) VALUES (?, ?,?);";
  if (php) {
    if (tphp === "begineer") {
  ...
  var sql3 =
    "INSERT INTO `preferances` (`employeeid`, `preferd_location`, `notice_period`, `ExpactedCTC`, `Department`, `currentctc`) VALUES (?,?,?,?,?,?);";
  await con
    .promise()
    .query(sql3, [e_id, location, notice, expactedCTC, department, currentCTC]);

  var sql1 = `SELECT * FROM basic_details where id=${e_id}`;
  con.query(sql1, (err, result) => {
    if (err) console.log(err);
    else {
      res.render("detailform", { result });
    }
  });
};
...
const postdetailform = async (req, res) => {
  //   app.post('/detailform/:id/update', async (req, res) => {
  var {
    fname,
    lname,
    designation,
  ...
  for (let i = 0; i < board.length; i++) {
    await con
      .promise()
      .query(sql1, [education[i], board[i], percentage[i], passingyear[i]]);
  }
  ...
  for (let i = 0; i < CompanyName.length; i++) {
    await con
      .promise()
      .query(sql2, [CompanyName[i], designation1[i], From1[i], To1[i]]);
  }
  ...
  for (let i = 0; i < nameref.length; i++) {
    var r = await con
      .promise()
      .query(sql3, [nameref[i], Contactnumber[i], Relation[i]]);
  }
  ...
  if (english) {
    ...
    await con
      .promise()
      .query(sql4, [6, english_r, english_w, english_s, lang[1]]);
  }
  ...
  if (gujarati) {
    ...
    await con
      .promise()
      .query(sql4, [7, gujarati_r, gujarati_w, gujarati_s, lang[2]]);
  }

  var { php, tphp, mysql, tmysql, oracle, toracle, laravel, tlaravel, tech } =
    req.body;
  ...
  await con
    .promise()
    .query(sql6, [location, notice, expactedCTC, department, currentCTC]);
};
module.exports = { postdetailform, getdetailform, detailform, getdetail };
