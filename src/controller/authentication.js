...
const generatetoken = async (req, res) => {
    try {
        ...
        const issue = await con
            .promise()
            .query("select * from registration where activation = ?", [activation]);
        result = issue[0][0];
        if (issue[0].length === 0)
            return res.send(`<body style="background-color: black;"><div style="text-align: center; background-color: #d7fcd7; height: 70px;width: 80%;margin-left: 100px;">
    <p style=" color: #0a4932;padding-top:20px"><b>token invalid<b></p></div></body>`);
        let updateacticvation = randomvalue(12);
        const updatedata = await con
            .promise()
            .query(
