...
const getsortidname = async (req, res) => {
  ...
  con.query(
    `SELECT * FROM student_master ORDER BY ${sortcolumn} ${sortorder} limit ${limit} offset ${offset}`,
...
const getattendance = async (req, res) => {
  ...
  con.query(sql, (err, result) => {
    ...
    else res.render("attendance", { result, p, m, last });
  });
};
...
const multisearch = async (req, res) => {
  ...
  try {
    if (studentid) {
      ...
      sql = `select * from student_master ${where} limit ${limit} offset ${offset}`;
    }
    con.query(sql, (err, result) => {
      if (err) console.log(err);
      else {
